...
const Cake = ({ w }) => {
  ...
  const [unitCost, setUnitCost] = useState({ unit: "", cost: 0 });
  const { cost } = unitCost;
  ...
  const handleChange = (e) => {
    ...
    let v = itmDelivCost.current.classList;
    if (v.value.includes("isCakeAlert")) {
      v.remove("isCakeAlert");
    }
    Swal.fire({
      position: "center",
      icon: "success",
  ...
  const handlePrice = (unit, cost, idx) => {
  ...
  const isUcActive = (arg) => {
    if (ucActive === arg) return " isCakeUcActive";
    return "";
  };
  return (
    <Fragment>
        ...
        <>
          {partItem && (
            <>
              ...
              <div className="isCakeHeight-1"></div>
              <div className="isCakeMainDiv">
                <div className="dFlexWrap justfyeven">
                  <div className="isCakeImgDiv">
                    <img
                      src={partItem.images[imgActive].url}
                      alt="..."
                      className="isCakeImgMain"
                    />
                    <div className="isCakeImagesList">
                      {partItem.images.map((imag, idx) => (
                        <img
                          ...
                          alt="..."
                          className={"isCakeImagesListImg" + isimgActive(idx)}
                        />
                      ))}
                    </div>
                  </div>
                  <div className="isCakeDetailsDiv">
                    <p className="isCakeTitle">{partItem.title}</p>
                    <div className="isCakeCost">
                      <Typography variant="button">
                        <span className="tomato itmCostValue">₹ {cost}</span>
                      </Typography>
                    </div>
                    <div className="isCakeRating">
                      <Rating
                        ...
                        readOnly
                      />
                      <p className="isCakenumReviews-1">
                        {partItem.numOfReviews} Reviews Yet
                      </p>
                    </div>
                    <div className="isCakeAddCartDiv">
                      <Button
                        variant="contained"
                        sx={{
                    ...
                    <p className="isCakeUnitsPara">Units:</p>
                    <div className="isCakeUnitsDiv">
                      {partItem.costByDeps.map((uc, idx) => (
                        <Chip
                          key={idx}
                          sx={{ margin: "2.5px 2px" }}
                          label={uc.unit}
                          onClick={() => handlePrice(uc.unit, uc.cost, idx)}
                          className={"isCakeUnitChip" + isUcActive(idx)}
                        />
                      ))}
                    </div>
                  </div>
                </div>
                <p className="isCakeDesc">{partItem.description}</p>
                <div className="isCakeDelivCost" ref={itmDelivCost}>
                ...
                <div className="isCakeReviewsDiv">
                  {partItem.numOfReviews === 0 ? (
                    <h5 className="isCakenoreview">No Reviews Yet</h5>
                  ) : (
