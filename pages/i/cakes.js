    ...
    <Fragment>
      {loading ? (
        ...
        <Fragment>
          {client && (
            <MetaData
              title={`Cakes near ${client.city} | www.partyondoor.com`}
            />
          )}
          <div>
...
export async function getServerSideProps(context) {
  ...
  const resp = await fetch(
    `${process.env.baseUrl}/api/v1/subcategory/specific?category=61a16dbddbcacd3e5a7271ec`
  );
  ...
  return {
    props: { w: "61a16dbddbcacd3e5a7271ec", subCats: data.subcategories }, // will be passed to the page component as props
  };
}
