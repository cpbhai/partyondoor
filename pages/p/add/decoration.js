...
const Create = () => {
    ...
    await fetch("/api/v1/category/gtone")
      ...
      .then((data) => {
        if (data.success) {
          ...
          categories = categories.filter(({ general }) => general == "decorat");
  ...
  const { title, desc, unit, cost } = values;
  const [costByDeps, setCostByDeps] = useState([]);
  const handleAddCost = () => {
    if (!unit || cost === "" || !desc)
      return dispatch(SendNotif("warning", "Invalid Unit/Cost/Description."));
    setCostByDeps([...costByDeps, { unit, cost: Number(cost), desc }]);
    setValues({ ...values, ["unit"]: "", ["cost"]: "", ["desc"]: "" });
  };
  ...
  return (
    <Fragment>
      ...
      <Typography
        ...
        gutterBottom
      >
        New Decoration Sample
      </Typography>
      <div className="pAddDecormainDiv">
        <Stack>
          <FormControl className="pAddDecorformControl">
            <Input
              type="text"
              placeholder="Title"
              className="pAddDecorinput"
              name="title"
              value={title}
          ...
          <div className="mt-1"></div>
          {costByDeps.length !== 0 ? (
            <div className="dFlexWrap justfyeven pAddDecorWidth">
              {costByDeps.map((data, idx) => (
                <Chip
                  key={idx}
                  label={`${data.unit}: ₹ ${
                    data.cost
                  } : ${data.desc.substring(0, 8)}`}
                  color="primary"
            ...
            <div className="foodItmUnitTipDiv">
              <Typography
                ...
                gutterBottom
              >
                Add Unit wise cost &amp; description like:
              </Typography>
              <Typography
                variant="caption"
                display="block"
                ...
                gutterBottom
              >
                no. of baloons, flowers OR Size of Area etc...
              </Typography>
            </div>
          )}
          <FormControl className="pAddDecorformControl">
            <Input
              type="text"
              placeholder={`200 baloons, 750 flowers, 500 sq.ft`}
              className="pAddDecorinput"
              sx={{ marginBottom: "9px" }}
              name="unit"
              value={unit}
          ...
          <FormControl className="pAddDecorformControl">
            <TextareaAutosize
              className="pAddDecorTextArea"
              placeholder={
                `Description` + (unit.length !== 0 ? ` For: ${unit}` : ``)
              }
              name="desc"
          ...
          <FormControl className="pAddDecorformControl">
            <label htmlFor="icon-button-file">
              ...
              {previews.length ? (
                <div className="dFlex justfycent pAddDecorDivPreviews">
                  {previews.map((img, idx) => (
                    <img
                      key={idx}
                      src={img}
                      alt="..."
                      className="pAddDecorimgPreview"
                    />
                  ))}
                </div>
              ) : (
                <span style={{ color: "darkgray" }}>
                  Add Images of Decoration(max. 3)
                </span>
              )}
            </label>
          </FormControl>
          ...
          <Link href="/p/dashboard">
            <a
              className="pAddDecorLink mt-2 textCenter fontwb"
              style={{ color: "red" }}
            >
              Cancel
            </a>
